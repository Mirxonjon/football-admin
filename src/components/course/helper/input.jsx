...
function InputsCourse () {
  ...
  const sent = async () => {
    ...
    if (titleValue  && description_trainingValue && description_training_ruValue && ageValue && fileImageValue) {
    ...
    formData.append('image', fileImageValue)
      ...
      await FT_API.post('trainingCategories/create', formData ,config ).then(data => {
        if(data.status == 201) {
          setCount(count + 1)
          setTimeout(() => {
            messageApi.open({
  ...
  return (
    <div className='inputs_course'>
      ...
      <ul>
        ...
        <li>
          <span>Yosh</span>
          <select  ref={age} defaultValue={'5-8'} name="age" id="">
