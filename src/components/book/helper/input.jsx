...
function InputVideo () {
  ...
  const sent = async() => {
    ...
    if (titleValue && title_ruValue  && CategoryValue && book_langValue && filePhoto && fileBook && description_bookValue && description_book_ruValue) {
      ...
      await FT_API.post('Books/create', formData ,config ).then(data => {
        if(data.status == 201) {
          setTimeout(() => {
            messageApi.open({
              key,
      ...
      <div className='inputs_course'>
        ...
        <ul>
          ...
          <li className='rasm'>
            ...
            <input id='bookFile' ref={book} className='none' type='file' />
          </li>

          <li className='rasm'>
            <span>Rasm</span>
            <label htmlFor='bookPhotoFile'>
              <i>{Tillar[0][lang].yukla}</i>
              <img src={yukla} alt='yukla' />
            </label>
            <input id='bookPhotoFile' ref={rasmi} className='none' type='file' />
          </li>
        </ul>
        {contextHolder}
