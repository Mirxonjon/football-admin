...
function ListBook () {
  ...
  useEffect(() => {
    FT_API.get('Books/all' ,config).then(e => {
      setVideo(e.data)
    })
    .catch(e=>{
  ...
  const bookDelete = async id => {
    ...
    await  FT_API.delete(`Books/delete/${id}` ,config ).then(data => {
      if(data.status == 204) {
        setTimeout(() => {
          messageApi.open({
            key,
      ...
      {video.length
        ? video?.map((e, i) => (
            <li style={{ backgroundColor: e?.course_bgc }} key={i}>
              <h2>{e?.title}</h2>
              <img src={img_link + e?.book_img} alt='course' />
              <a href={img_link + e?.book_link} target="_blank" rel="noopener noreferrer"> 
              <Button type="primary"  download={img_link + e?.book_link} ghost style={{ width: '100%' ,margin:'20px 0px' }}>Kitobni ko'rish</Button>
              </a>
              <p>
