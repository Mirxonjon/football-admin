...
const ModalVideo = () => {
  ...
  const handleOk = async () => {
    ...
    formData.append('book', fileBook)
    formData.append('image', filePhoto)
    formData.append('title', titleValue)
    formData.append('title_ru', title_ruValue)
    formData.append('book_lang', book_langValue)
    ...
    await FT_API.patch(`Books/update/${videoModal?.id}`, formData ,config ).then(data => {
      if(data.status == 204) {
        setCount(count + 1)
        setTimeout(() => {
          messageApi.open({
  ...
  return (
    <>
      ...
      <li>
          ...
          <li>
            <span>Kitob haqida qisqacha ma’lumot</span>
            <input ref={description_book} type='text' placeholder='Kitob haqida' />
          </li>
          <li>
            <span>Kitob haqida qisqacha ma’lumota rus tilida</span>
            <input ref={description_book_ru} type='text' placeholder='rus tilida' />
          </li>
          <li className='rasm'>
            <span>Kitob</span>
            <label htmlFor='bookFileEdit'>
              <i>{Tillar[0][lang].yukla}</i>
              <img src={yukla} alt='yukla' />
            </label>
            <input id='bookFileEdit' ref={book} className='none' type='file' />
          </li>

          <li className='rasm'>
            <span>Rasm</span>
            <label htmlFor='bookPhotoFileEdit'>
              <i>{Tillar[0][lang].yukla}</i>
              <img src={yukla} alt='yukla' />
            </label>
            <input id='bookPhotoFileEdit' ref={rasmi} className='none' type='file' />
          </li>
        </ul>
