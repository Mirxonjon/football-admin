...
function ListCourse () {
  ...
  useEffect( () => {
   FT_API.get('MasterclassCategory/all').then(e => {
      setData(e.data)
    })
  }, [setData, count, token])
  ...
  const CourseDelete = async id => {
    ...
    await  FT_API.delete(`MasterclassCategory/delete/${id}` ,config ).then(data => {
      if(data.status == 204) {
        setCount(count + 1)
        setTimeout(() => {
          messageApi.open({
        ...
        {data.length
          ? data.map((e, i) => (
              <li style={{ backgroundColor: e?.course_bgc }} key={i}>
                <h2>{e?.title}</h2>
                <img src={img_link + e?.img_link} alt='course' />
                <p>
                  <span>Sarlavha rus tilida</span> <p>{e?.title_ru}</p>
                </p>
                <p>
                  <span>Master-klass haqida qisqacha ma’lumot:</span> <p>{e?.title_descrioption} </p>
                </p>
                <p>
                  <span>Master-klass haqida qisqacha ma’lumot rus tilida:</span> <p>{e?.title_descrioption_ru} </p>
                </p>
