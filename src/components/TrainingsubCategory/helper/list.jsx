...
function ListCourse () {
  ...
  useEffect( () => {
   FT_API.get('trainingSubCategories/all').then(e => {
      setData(e.data)
    })
  }, [setData, count, token])
  ...
  const CourseDelete = async id => {
    ...
    await  FT_API.delete(`trainingSubCategories/delete/${id}` ,config ).then(data => {
      if(data.status == 204) {
        setCount(count + 1)
        setTimeout(() => {
          messageApi.open({
        ...
        {data.length
          ? data.map((e, i) => (
              <li style={{ backgroundColor: e?.course_bgc }} key={i}>
                ...
                <p>
                  <span>Mo'ljalangan yosh:</span> <p>{e?.category_id.traning_for_age} </p>
                </p>
                <p>
                  <span>Mashg'ulotlar katta gruppasi nomi</span> <p>{e?.category_id.title} </p>
                </p>
