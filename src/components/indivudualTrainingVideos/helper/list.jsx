...
function ListVideo () {
  ...
  useEffect(() => {
    FT_API.get('IndividualTrainingVideos/all' ,config).then(e => {
      setVideo(e.data)
    })
    .catch(e=>{
  ...
  const videoDelete = async id => {
    ...
    await  FT_API.delete(`IndividualTrainingVideos/delete/${id}` ,config ).then(data => {
      if(data.status == 204) {
        setTimeout(() => {
          messageApi.open({
            key,
      ...
      {video.length
        ? video?.map((e, i) => (
            <li style={{ backgroundColor: e?.course_bgc }} key={i}>
              ...
              <p>
                <span>Kichik Guruhi :</span> <p>{e?.category_id.title}</p>
              </p>
              <b>
                <EditOutlined
