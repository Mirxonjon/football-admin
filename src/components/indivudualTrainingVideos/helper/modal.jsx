...
const ModalVideo = () => {
  ...
  const handleOk = async () => {
    ...
    await FT_API.patch(`IndividualTrainingVideos/update/${videoModal?.id}`, formData ,config ).then(data => {
      if(data.status == 204) {
        setCount(count + 1)
        setTimeout(() => {
          messageApi.open({
  ...
  return (
    <>
      ...
      <li>
          ...
          <li className='rasm'>
            <span>Video</span>
            <label htmlFor='indivudialvideoFiletrain'>
              <i>{Tillar[0][lang].yukla}</i>
              <img src={yukla} alt='yukla' />
            </label>
            <input id='indivudialvideoFiletrain' ref={video} className='none' type='file' />
          </li>

          <li className='rasm'>
            <span>Rasm</span>
            <label htmlFor='phototrainIndivudual'>
              <i>{Tillar[0][lang].yukla}</i>
              <img src={yukla} alt='yukla' />
            </label>
            <input id='phototrainIndivudual' ref={rasmi} className='none' type='file' />
          </li>
        </ul>
