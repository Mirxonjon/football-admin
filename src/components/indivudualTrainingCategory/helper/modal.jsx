...
const ModalApp = () => {
  ...
  const handleOk = async() => {
    ...
    await FT_API.patch(`IndividualtrainingCategories/update/${openModal?.id}`, formData ,config ).then(data => {
      if(data.status == 204) {
        setCount(count + 1)
        setTimeout(() => {
          messageApi.open({
  ...
  return (
    <>
      ...
      <ul>
        ...
        <li className='rasm'>
          <span>Rasm</span>
          <label htmlFor='IndividualTraininggrouprasmUpdate'>
            <i>Yuklash</i>
            <img src={yukla} alt='yukla' />
          </label>
          <input id='IndividualTraininggrouprasmUpdate' ref={image} className='none' type='file' />
        </li>
      </ul>
