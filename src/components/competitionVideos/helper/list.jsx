...
function ListVideo () {
  ...
  useEffect(() => {
    FT_API.get('competitionVideos/all' ,config).then(e => {
      setVideo(e.data)
    })
    .catch(e=>{
  ...
  const videoDelete = async id => {
    ...
    await  FT_API.delete(`competitionVideos/delete/${id}` ,config ).then(data => {
      if(data.status == 204) {
        setTimeout(() => {
          messageApi.open({
            key,
  ...
  return (
    <div className='list_course'>
      {contextHolder}
    <h1>Mavjud musobaqa videolari</h1>
    <ul className='top'>
      {video.length
        ? video?.map((e, i) => (
            <li style={{ backgroundColor: e?.course_bgc }} key={i}>
              <h2>{e?.title}</h2>
              <YouTube videoId={e?.video_link?.split('v=')[1]}   />
              <p>
                <span>Sarlavha rus tilida</span> <p>{e?.title_ru}</p>
              </p>
              <p>
                <span>Video haqida qisqacha ma’lumot:</span> <p>{e?.description_video}</p>
              </p>
              <p>
                <span>Video haqida qisqacha ma’lumot rus tilida:  </span> <p>{e?.description_video_ru}</p>
              </p>
              <p>
                <span>Guruhi :</span> <p>{e?.category_id.title}</p>
              </p>
              <b>
                <EditOutlined
