...
import { img_link } from '../../../utils/api'
...
function ListCourse () {
  ...
  useEffect( () => {
   FT_API.get('competitionCategories/all').then(e => {
      setData(e.data)
    })
  }, [setData, count, token])
  ...
  const CourseDelete = async id => {
    ...
    await  FT_API.delete(`competitionCategories/delete/${id}` ,config ).then(data => {
      if(data.status == 204) {
        setCount(count + 1)
        setTimeout(() => {
          messageApi.open({
  ...
  return (
    <div className='list_course'>
      <h1>Mavjud Musobaqalar guruhlari </h1>
      <ul className='top'>
        {data.length
          ? data.map((e, i) => (
